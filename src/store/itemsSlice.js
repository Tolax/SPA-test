      ...
      try {
        const response = await axios.get('https://dummyjson.com/recipes', {
          params: {
            limit: 0,
            skip: 0,
            select: 'name,image,instructions,difficulty,mealType,cuisine,cookTimeMinutes,prepTimeMinutes,tags'
          }
        });
...
export const fetchCuisines = createAsyncThunk(
    ...
    async () => {
        try {
            const response = await fetch("https://dummyjson.com/recipes?limit=50&skip=0&select=name,cuisine");
            const data = await response.json();

            if (Array.isArray(data.recipes) && data.recipes.length > 0 && data.recipes[0].cuisine) {
                const cuisinesSet = new Set(data.recipes.map((recipe) => recipe.cuisine));
...
export const fetchMealTypes = createAsyncThunk(
    ...
    async () => {
        try {
            const response = await fetch("https://dummyjson.com/recipes?limit=50&skip=0&select=name,mealType");
            const data = await response.json();

            if (Array.isArray(data.recipes) && data.recipes.length > 0 && data.recipes[0].mealType) {
